...
import snoozeNotificationForToday from '@salesforce/apex/NavatarNotificationCtrl.snoozeNotificationForToday';
...
export default class NavatarNotificationPopupLwc extends NavigationMixin(LightningElement) {
    ...
    // to fetch user visibility to popup visible or not?
    notificationVisibilityStatus(){
        checkNotificationVisibleForuser()
        .then((result) => {
                if(result){
                ...
                if(result.length > 0){
                    ...
                    this.notificationRecords = allNotifications;
                            ...
                            if(this.notificationCount > 0){// Code Review Feedback 
                                this.isShowModal= true; //Bug # 00043027 & 00043026
                            }
    ...
    // snooze notification for today when click to "Snooze for Today" button 
    // will be snooze notification for the day 
    snoozeForToday(){
        this.isLoading = true;
        snoozeNotificationForToday()
        .then((result) => {
            this.showNotification('Success', 'Your Notification has been Snoozed for a Day.', 'Success');
