...
import { showNotification, validateAdvanceFilterLogic } from "./navatarFilterFieldsLwcHelper"; // To import methods from another file

export default class NavatarFilterFieldsLwc extends LightningElement {
        ...
        if (objectInfo.fields) {
            ...
            for (let key in fieldsInfo) {
                if (!(this.ignoreTheseTypes.includes(fieldsInfo[key].apiName.toUpperCase())
                    ...
                    itemList.push({
                        objName: objectInfo.custom ? objectInfo.label : objectInfo.apiName,
                        label: fieldsInfo[key].label,
                        displayName: ((objectInfo.custom ? objectInfo.label : objectInfo.apiName) + ':' + fieldsInfo[key].label).replace(/\s/g, ' '),
    ...
    handleRowInsertion() {
        if (this.filterList.length >= 1) {
            ...
            let temp = this.template.querySelectorAll("c-navatar-expression-builder-lwc");
            let index = 0;
    ...
    handleApplyFilter() {
        ...
        for (let count = 0; count < this.rowSize; count++) {
            console.log('temp++'+temp[count].compData.fieldName);
        ...
        console.log('temp++'+JSON.stringify(filterData));
