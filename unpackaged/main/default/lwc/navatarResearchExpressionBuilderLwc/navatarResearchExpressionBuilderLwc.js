...
import setFilter from './NavatarResearchExpressionBuilderLwcHelper';
import {
    ShowToastEvent
  } from 'lightning/platformShowToastEvent';

export default class  NavatarResearchExpressionBuilderLwc extends LightningElement {
    boolVar;
    @api rowSize;
    @api fieldStruct;
    @api index;
    ...
    renderedCallback() {
        if (this.template.querySelector('c-navatar-Research-Combobox-With-Search-Lwc').searchval != this.compData.searchVal) {
            this.template.querySelector('c-navatar-Research-Combobox-With-Search-Lwc').searchval = this.compData.searchVal
    ...
    */
    handleFilterChange(event) {
        ...
        if(this.compData.fDataType == 'REFERENCE' && (this.compData.filter == '=' || this.compData.filter == '!=' || this.compData.filter == 'like start' || this.compData.filter == '>' || this.compData.filter == '<' || this.compData.filter == '<=' || this.compData.filter == '>=' || this.compData.filter == 'like==' || this.compData.filter == 'not like!==')){
    ...
    handleOptionSelect(event) {
        ...
        res.objName = selectedList.objApiName;//selectedList.displayName.split(':')[0];
